...
function doors.register_trapdoor(name, def)
	...
	def.on_rightclick = function (pos, node)
		...
		if node.name == name_closed then
			sound = def.sound_open
			if def.on_open then def.on_open(pos, node) end
		end
		if node.name == name_opened then
			sound = def.sound_close
			if def.on_close then def.on_close(pos, node) end
		end
		if sound then
	...
	if def_opened.groups == nil then def_opened.groups = {} end
